...
{ "ER_TRG_ON_VIEW_OR_TEMP_TABLE", 1361, "Trigger\'s \'%-.192s\' is view, temporary table or sequence" },
...
{ "ER_COL_COUNT_DOESNT_MATCH_PLEASE_UPDATE_V2", 3009, "Column count of %s.%s is wrong. Expected %d, found %d. Created with MariaDB %d, now running %d. Please use mysql_upgrade to fix this error." },
...
{ "ER_COMMULTI_BADCONTEXT", 4000, "COM_MULTI can\'t return a result set in the given context" },
{ "ER_BAD_COMMAND_IN_MULTI", 4001, "Command \'%s\' is not allowed for COM_MULTI" },
...
{ "ER_UNSUPPORT_COMPRESSED_TEMPORARY_TABLE", 4047, "CREATE TEMPORARY TABLE is not allowed with ROW_FORMAT=COMPRESSED or KEY_BLOCK_SIZE." },
...
{ "ER_WRONG_INSERT_INTO_SEQUENCE", 4093, "Wrong INSERT into a SEQUENCE. One can only do single table INSERT into a sequence object (like with mysqldump).  If you want to change the SEQUENCE, use ALTER SEQUENCE instead." },
...
{ "WARN_VERS_PART_FULL", 4114, "Versioned table %`s.%`s: partition %`s is full, add more HISTORY partitions" },
